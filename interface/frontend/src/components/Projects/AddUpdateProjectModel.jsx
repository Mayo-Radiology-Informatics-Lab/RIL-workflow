...
export const AddUpdateProjectModel = ({
  editable = false,
  defaultValues = {},
  onSuccess = () => {},
  ...rest
  ...
  const onSubmit = async (values) => {
    try {
      if (editable) {
        ...
        await axiosInstance.post(`/projects/create/`, values);
      }
      toast({
        title: editable ? "Project Updated" : "Project Added",
        status: "success",
        isClosable: true,
        duration: 1500,
      });
  ...
  return (
    <Box {...rest}>
      <Button w="100%" colorScheme="green" onClick={onOpen}>
        {editable ? "UPDATE FODO" : "ADD PROJECT"}
      </Button>
      <Modal
      ...
      >
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <ModalContent>
            <ModalHeader>{editable ? "Update Project" : "ADD PROJECT"}</ModalHeader>
            <ModalCloseButton />
            <ModalBody>
